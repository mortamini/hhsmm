...
dmixmvnorm <- function(x, j, model){
	...
	if(length(model$parms.emission$mix.p[[j]])>1){
		...
		for(i in 1:k){
			...
			if(d>1){
 				dens = dens + model$parms.emission$mix.p[[j]][i]*dmvnorm(x,mean = model$parms.emission$mu[[j]][[i]],
                               sigma = model$parms.emission$sigma[[j]][[i]])
			} else {
				dens = dens + model$parms.emission$mix.p[[j]][i]*dnorm(x,model$parms.emission$mu[[j]][[i]],
